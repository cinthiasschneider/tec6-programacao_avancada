...
pygame.display.set_caption("Navegação - Comunicação direta")
...
# usando padrão de projeto mediator para fazer a comunicação direta entre os agentes
class TrafficMediator:
    def __init__(self):
        self.reservations = {} # reserva de locações

    def negotiate(self, agent, spot, step):
        # verifica se outro agente já comunicou intenção de ocupar o espaço
        occupant = self.reservations.get((step, spot))
        if occupant is None:
            self.reservations[(step, spot)] = agent
            return True
        
        # negociação baseada em prioridade (level aleatório)
        if agent.level > occupant.level:
            self.reservations[(step, spot)] = agent
            return True
        return False
...
class ExecutionManager(Observer):
    ...
    def initialize(self, win, width):
        ...
        # mediator de comunicação direta
        self.mediator = TrafficMediator()
    ...
    def schedule_move_commands(self):
        self.scheduled_commands = []
        self.executed_commands_history = []
        self.mediator.reservations = {} # reinicia as negociações
        ...
        for agent in temp_agents:
            ...
            agent.current_spot_sim = agent.start
            agent.active = True

        step_idx = 0
        while any(a.active for a in temp_agents) and step_idx < 1500:
            step_idx += 1
            for agent in temp_agents:
                if not agent.active: 
                    continue
                ...
                # agentes negociam a entrada no próximo movimento
                if self.mediator.negotiate(agent, next_target, step_idx):
                    self.scheduled_commands.append((current_time + step_idx * time_step, MoveAgentCommand(agent, next_target, agent.current_spot_sim)))
                    agent.current_spot_sim = next_target
                    agent.path_index -= 1
                    if agent.current_spot_sim == agent.end: agent.active = False
                else:
                    # WaitCommand caso perca a negociação
                    self.mediator.reservations[(step_idx, agent.current_spot_sim)] = agent
                    self.scheduled_commands.append((current_time + step_idx * time_step, WaitCommand(agent, agent.current_spot_sim)))
            ...
            if MODO_ESCOLHIDO == '2':
                 ...
                 for a in manager.all_agents:
                     ...
                     if base_a.current_spot == self:
                         ...
                         break
                 if base_agent_in_list and hasattr(base_agent_in_list, 'level'):
                    FONT_LEVEL = pygame.font.Font(None, 16)
                    # usa o level do agente base
                    level_text = FONT_LEVEL.render(str(base_agent_in_list.level), True, WHITE if self.color == BLACK else BLACK)
                    win.blit(level_text, (agent_pos[0] - level_text.get_width() // 2, agent_pos[1] - level_text.get_height() // 2))
...
class Agente(SearchComponent, Subject):
    def __init__(self, start, end, color, single_mode=False, algorithm_factory=None):
        ...
        # random para comunicação e level dos agentes
...
def generate_random_agents(grid, num_agents, factory, algorithm_factory):
    ...
    while len(agents) < num_agents:
        ...
        if not start.is_barrier() and start != end and start not in used_start:
            ...
            if agent.find_path(grid):
                agents.append(agent); used_start.add(start)
                for r in grid:
                    for s in r:
    ...
    for a in agents: a.start.make_agent_pos(a.color); a.end.make_end(a.color)
    return agents
