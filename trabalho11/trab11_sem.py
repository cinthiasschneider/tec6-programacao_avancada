...
pygame.display.set_caption("Navegação - Sem comunicação")
...
def get_rvo_velocities(manager):
    ativos = [a for a in manager.all_agents if get_base_agent(a).active]
    if not ativos: return []

    X_list, V_desired_list = [], []
    for agent in ativos:
        base = get_base_agent(agent)
        X_list.append([float(base.current_spot.col), float(base.current_spot.row), 0.0, 0.0, AGENT_RADIUS]) # passa posição atual
        
        # mira no destino final ou 5 células à frente no caminho pra não dar deadlock no rvo!!!
        if base.path_index >= 0:
            target_idx = max(0, base.path_index - 5)
            target = base.path[target_idx]
        else:
            target = base.end

        vx = float(target.col - base.current_spot.col)
        vy = float(target.row - base.current_spot.row)
        dist = (vx**2 + vy**2)**0.5
        
        if dist > 0:
            V_MAX = 1.0
            V_desired_list.append([(vx/dist) * V_MAX + 0.1, (vy/dist) * V_MAX]) # bias lateral pra evitar batidas
        else:
            V_desired_list.append([0.0, 0.0])
 
    obs_list = [] # passando obstáculos
    for row in manager.grid:
        for cell in row:
            if cell.is_barrier():
                obs_list.append([float(cell.col), float(cell.row), 1.0, 1.0])

    ws_model = {'robot_radius': AGENT_RADIUS, 
                'circular_obstacles': [], 
                'square_obstacles': obs_list, 
                'bounds': [0, ROWS, 0, ROWS]}
    
    return RVO_update(np.array(X_list), np.array(V_desired_list), obs_list, ws_model)
...
class ExecutionManager(Observer):
    ...
    def schedule_move_commands(self):
        ...
        # reseta o estado dos agentes para o início
        for agent in self.all_agents:
            base = get_base_agent(agent)
            base.path_index = len(base.path) - 1
            base.active = True
    ...
    def execute_scheduled_commands(self):
        ...
        agentes_ativos = [a for a in self.all_agents if get_base_agent(a).active]
        if not agentes_ativos: 
            self.running = False
            return

        new_vs = get_rvo_velocities(self)
    
        for i, agent in enumerate(agentes_ativos):
            base = get_base_agent(agent)
            vx, vy = new_vs[i]
        
            if abs(vx) > 0.1 or abs(vy) > 0.1:
                next_col = int(round(base.current_spot.col + vx))
                next_row = int(round(base.current_spot.row + vy))
            
                # definindo limites do grid
                next_col = max(0, min(next_col, ROWS - 1))
                next_row = max(0, min(next_row, ROWS - 1))
                target_cell = self.grid[next_row][next_col]
            
                if target_cell != base.current_spot and not target_cell.is_barrier():
                    base.current_spot.is_agent = False # desmarcar a célula anterior antes de mover, pra abrir caminho
                
                    cmd = MoveAgentCommand(base, target_cell, base.current_spot)
                    cmd.execute()
                
                    # usando index pra checagem
                    if base.path_index >= 0:
                        for check_idx in range(max(0, base.path_index - 10), base.path_index + 1):
                            p = base.path[check_idx]
                            if abs(target_cell.col - p.col) <= 1 and abs(target_cell.row - p.row) <= 1:
                                base.path_index = check_idx - 1
                                break

            # verifica a chegada do agente ao final
            if abs(base.current_spot.col - base.end.col) <= 1 and abs(base.current_spot.row - base.end.row) <= 1:
                base.active = False
                base.current_spot.make_end(base.color)
...
class Agente(SearchComponent, Subject):
    def __init__(self, start, end, color, single_mode=False, algorithm_factory=None):
        ...
        # random para comunicação e hp dos agentes
