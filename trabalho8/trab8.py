...
pygame.display.set_caption("Navegação - Padrões de Projeto")
        ...
        while any(agent.path_index >= 0 for agent in temp_agents):
            for decorated_agent in self.all_agents:
                 ...
                 if agent.running and agent.path_index >= 0:
                    command = agent.create_move_command() 
                    if command:
...
class ModoHandler(ConfiguracaoHandler):
    def _process(self, win, width, manager):
        ...
        print(f"Configuração: Modo {MODO_ESCOLHIDO} selecionado (Agentes: {num_agents_procedural}).")
        return True
        ...
        if self.is_agent:
            ...
            if MODO_ESCOLHIDO == '2':
                 ...
                 for a in manager.all_agents:
                     if get_base_agent(a).current_spot == self:
                         base_agent_in_list = get_base_agent(a)
                         break
...
class Agente(SearchComponent, Subject):
    ...
    def take_damage(self):
        self.life -= 1
        print(f"Agente {self.color} sofreu dano. Vida restante: {self.life}")
...
def draw(win, grid, rows, width):
    ...
    text_info = f"Algoritmo: {algo_display} | Grid: {geo_display} | A: A* | D: DFS | K: Dijkstra | C: limpar | Z: desfazer"
    if MODO_ESCOLHIDO == '1':
