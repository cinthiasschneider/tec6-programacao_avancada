...
class ExecutionManager(Observer):
    ...
    # interface observer => remove o agente morto e agenda novamente
    def update(self, subject: Agente):
        if isinstance(subject, Agente) and subject.is_dead:
            print(f"AGENTE {subject.color} MORREU!")
            
            # remove o agente da lista de agentes ativos
            self.all_agents = [a for a in self.all_agents if get_base_agent(a) != subject]
            
            # reseta as células de início e fim do agente morto
            subject.start.reset()
            subject.end.reset()

            if MODO_ESCOLHIDO == '2':
                print(f"Agente {subject.color} removido. Agentes restantes: {len(self.all_agents)}")
                
                # se ainda houver agentes e a simulação estava rodando => reagenda
                if self.all_agents and self.running:
                    # necessário limpar as células do grid que ele ocupava
                    for row in self.grid:
                        for spot in row:
                            if spot.agent_color == subject.color:
                                spot.reset()
                    
                    self.schedule_move_commands()
                    self.running = True
                elif not self.all_agents:
                    print("Todos os agentes morreram! Resetando o ambiente...")
                    self.reset_state() 
            
            elif MODO_ESCOLHIDO == '1':
                print("Agente interativo falhou! Resetando o ambiente...")
                self.reset_state()
    ...
    def reset_state(self):
        ...
        if MODO_ESCOLHIDO == '2':
            ...
            if ALGORITHM_CHOICE == 'ASTAR':
                ...
                current_base_factory = self.dfs_factory
            elif ALGORITHM_CHOICE == 'DIJKSTRA':
                current_base_factory = self.dijkstra_factory
...
# command para executar a movimentação dos agentes
    def schedule_move_commands(self):
        self.scheduled_commands = []
        self.executed_commands_history = []
        current_time = pygame.time.get_ticks()
        time_step = 50 
        temp_agents = [get_base_agent(a) for a in self.all_agents if isinstance(get_base_agent(a), Agente)]
        ...
        # gera todos os comandos sequenciais da rota completa
        while any(agent.path_index >= 0 for agent in temp_agents):
            for decorated_agent in self.all_agents:
                 agent = get_base_agent(decorated_agent)
                 if agent.running and agent.path_index >= 0:
                    # cria o comando => detecção preditiva de caminhos
                    command = agent.create_move_command_for_schedule() 
                    
                    if command:
                        current_time += time_step
                        self.scheduled_commands.append((current_time, command))
                    elif agent.path_index < 0:
                         agent.running = False 

        # reseta os agentes para a posição inicial => simulação começa do zero
        for agent in self.all_agents:
            base_agent = get_base_agent(agent)
            base_agent.path_index = len(base_agent.path) - 1 
            base_agent.current_spot = base_agent.start 
            base_agent.running = True 

    def execute_scheduled_commands(self):
        current_time = pygame.time.get_ticks()
        while self.scheduled_commands and self.scheduled_commands[0][0] <= current_time:
            _, command_to_execute = self.scheduled_commands.pop(0)
            
            # command verifica a colisão imediata
            if command_to_execute.execute():
                self.executed_commands_history.append(command_to_execute)
            else:
                # se colisão, o agente envolvido para de rodar e notifica => reset
                pass 
                
        if not self.scheduled_commands and self.running:
            self.running = False
            for agent in self.all_agents:
                 get_base_agent(agent).running = False

    def undo_last_command(self):
        if self.executed_commands_history:
            last_command = self.executed_commands_history.pop()
            last_command.undo()
            self.running = False 
            return True
        return False
...
class MoveAgentCommand(AgentCommand):
    def __init__(self, agent, next_spot, prev_spot):
        ...
        self.original_color_prev_spot = self.prev_spot.color
        self.original_is_agent_prev_spot = self.prev_spot.is_agent
        self.original_agent_color_prev_spot = self.prev_spot.agent_color
        
    def execute(self):
        # verifica se o próximo passo está ocupado por outro agente
        if self.next_spot != self.agent.end and self.next_spot.is_agent:
            other_agent_color = self.next_spot.agent_color
            print(f"COLISÃO REAL DETECTADA! Agente {self.agent.color} colidiu com agente {other_agent_color} em {self.next_spot.get_pos()}.")
                        
            # 1. agente que estava em movimento sofre dano
            print(f" -> Agente {self.agent.color} sofreu dano!")
            self.agent.take_damage()
            
            # 2. encontra o agente parado no caminho e aplica dano
            manager = ExecutionManager()
            for decorated_agent in manager.all_agents:
                base_agent = get_base_agent(decorated_agent)
                
                # verifica se o agente na célula é diferente de 'self.agent' e está no mesmo spot
                if (base_agent.current_spot == self.next_spot and 
                    base_agent.color == other_agent_color and 
                    base_agent != self.agent):
                    
                    print(f" -> Agente {base_agent.color} [parado] sofreu dano!")
                    base_agent.take_damage()
                    break
            return False 
        ...
        if self.next_spot == self.agent.end and self.agent.path_index < 0:
             self.agent.take_damage() 
             
        return True

    def undo(self):
        if self.next_spot != self.agent.end:
            self.next_spot.reset()
        else:
            self.next_spot.is_agent = False
            self.next_spot.agent_color = None
            self.next_spot.color = self.next_spot.original_color 
            
        self.prev_spot.color = self.original_color_prev_spot
        self.prev_spot.is_agent = self.original_is_agent_prev_spot
        self.prev_spot.agent_color = self.original_agent_color_prev_spot
        ...
        print(f"Desfazer executado na posição {self.agent.current_spot.get_pos()}")
        return True
...
class Agente(SearchComponent, Subject):
    ...
    def take_damage(self):
        self.life -= 1
        print(f"Agente {self.color} sofreu dano. Vida: {self.life}")
        if self.life <= 0 and not self.is_dead:
            self.is_dead = True
            self.notify() 
